...
console.log(args[0])}
if (!fs.existsSync('./JadiBotSessions/'+ id)){
fs.mkdirSync('./JadiBotSessions/'+ id, { recursive: true })}
args[0] && args[0] != undefined ? fs.writeFileSync('./JadiBotSessions/' + id + '/creds.json', JSON.stringify(JSON.parse(Buffer.from(args[0], 'base64').toString('utf-8')), null, '\t')) : ''
if (fs.existsSync('./JadiBotSessions/' + id + '/creds.json')) {
let creds = JSON.parse(fs.readFileSync("./JadiBotSessions/" + id + "/creds.json"))
if (creds) {
if (creds.registered = false) {
fs.unlinkSync('./JadiBotSessions/' + id + '/creds.json')
}}}
...
args[0] ? fs.writeFileSync('./JadiBotSessions/' + id + '/creds.json', JSON.stringify(JSON.parse(Buffer.from(args[0], 'base64').toString('utf-8')), null, `\t`)) : ''
let { version, isLatest } = await fetchLatestBaileysVersion()
const msgRetry = (MessageRetryMap) => { }
const msgRetryCache = new NodeCache()
const { state, saveState, saveCreds } = await useMultiFileAuthState("./JadiBotSessions/" + id)
...
console.log(reason)
if (reason == 405) {
await fs.unlinkSync('./JadiBotSessions/' + id + '/creds.json')
return await conn.reply(m.chat, '✧ Cerrando.', fkontak)
}
if (reason === DisconnectReason.restartRequired) {
jddt()
return console.log('✧ Conexión reemplazada.')
} else if (reason === DisconnectReason.loggedOut) {
sleep(4000)
return conn.reply(m.chat, '✧ Conexión cerrada.', fkontak)
} else if (reason == 428) {
await endSesion(false)
return conn.reply(m.chat, '✧ Conexión cerrada, reconectando.', fkontak)
} else if (reason === DisconnectReason.connectionLost) {
await jddt()
return console.log('✧ Conexión perdida con el Servidor, reconectando.')
} else if (reason === DisconnectReason.badSession) {
return await conn.reply(m.chat, '✧ Conexión cerrada, vuelva a conectarse.', fkontak)
} else if (reason === DisconnectReason.timedOut) {
await endSesion(false)
return console.log('✧ Tiempo de conexión agotada, reconectando.')
} else {
console.log('✧ Razón de la Desconeccion Desconocida ${reason || ""} ⪼ ${connection || ""}')
}}
...
console.log(conn.chats, `✧ Procesando mensajes.`, conn.ev)
Object.keys(conn.chats).forEach(jid => {
