...
filesDeleted++;
console.log(`Eliminacion de AlisaSession (PreKey) que provocan el undefined el Chat.`)}}}

if (chat.detect2 && m.messageStubType == 21) {
await this.sendMessage(m.chat, { text: `❀ ${usuario} cambió el nombre del grupo.`, mentions: [m.sender], mentions: (await conn.groupMetadata(m.chat)).participants.map(v => v.id) }, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100}) 

} else if (chat.detect2 && m.messageStubType == 22) {
await this.sendMessage(m.chat, { text: `❀ ${usuario} cambió la imagen del grupo.`, mentions: [m.sender] }, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100}) 
...
await this.sendMessage(m.chat, { text: `❀ Ahora ${m.messageStubParameters[0] == 'on' ? 'solo Administradores' : 'todos'} pueden editar la informacìón del grupo.`, mentions: [m.sender] }, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})

} else if (chat.detect2 && m.messageStubType == 26) {
await this.sendMessage(m.chat, { text: `❀ El grupo ha sido ${m.messageStubParameters[0] == 'on' ? 'cerrado' : 'abierto'}\n\n> → ${m.messageStubParameters[0] == 'on' ? 'Solo Administradores' : 'Ahora todos'} pueden enviar mensajes.`, mentions: [m.sender] }, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})
...
await this.sendMessage(m.chat, { text: `❀ ${usuario} se cambió la configuración de los Mensajes Temporales a *${m.messageStubParameters[0]}*`, mentions: [m.sender] }, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})
