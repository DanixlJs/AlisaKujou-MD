...
const ownerNum = '595983799436@s.whatsapp.net'
...
│ *❀ 𝐈𝐍𝐅𝐎 𝐁𝐎𝐓 ʚ♡⃛ɞ(ू•ᴗ•ू❁)*
├────────────────────
│👑 𝐂reador: @${ownerNum.replace('@s.whatsapp.net', '').trim()}
│📌 𝐓ipo: %sbot
│✏️ 𝐕ersión: ${global.vs}
│⏱️ 𝐀ctiva: %muptime
...
`.trimStart(),
  ...
  after: `> ${global.dev}`,
    ...
    let rtotalreg = Object.keys(users).filter(user => users[user].registrado).length;
  ...
  m.react('🤍')
await conn.sendMessage(m.chat, { video: gif, gifPlayback: true, caption: text.trim(), mentions: conn.parseMention(text), contextInfo: {
mentionedJid: await conn.parseMention(text),
isForwarded: true,
forwardingScore: 1, 
forwardedNewsletterMessageInfo: {
newsletterJid: '120363302120826314@newsletter',
