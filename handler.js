...
if (!isNumber(user.dinero)) user.dinero = 500;
...
if (!isNumber(user.dailytime)) user.dailytime = -1;
...
if (!('antinavez' in chat)) chat.antiunavez = false;
...
if (m.text && user.baneado && !isROwner && !isOwner && !isMods) {
...
const danixljs = `${plugin.botAdmin || plugin.admin || plugin.group || plugin || noPrefix || hl || m.text.slice(0, 1) == hl || plugin.command}`;
if (adminMode && !isOwner && !isROwner && m.isGroup && !isAdmin && danixljs) return;
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || '❀  Bienvenido 「 @user 」,').replace('@subject', await m.conn.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Sin descripción') : (chat.sBye || this.bye || conn.bye || '✧ Hasta luego 「 @user 」,')).replace('@user', '@' + user.split('@')[0]);
