      ...
      if (typeof plugin.all === 'function') {
        try {
          ...
          /* for (const [jid] of global.reportes_solicitudes.filter(([number]) => number)) {
            ...
            if (data.exists) {
              await m.reply(`- *ERROR DE COMANDO*\n\n- *Plugin:* ${name}\n- *Usuario:* ${m.sender}\n- *Texto:* ${m.text}\n\n- *Error:*\n\`\`\`${format(e)}\`\`\`\n\n> Reportelo con el comando *${usedPrefix}reporte <texto>*`.trim(), data.jid);
            }
          }*/
...
const messageText = `
- *USUARIO BANEADO*
- *Usuario:* ${messageNumber} ${user.bannedReason ? `\n- *Motivo:* ${user.bannedReason}` : '- *Motivo:* no especificado.'}\n\n> Puedes apelar tu Baneo en wa.me/595983799436`.trim();
              m.reply(messageText);
        ...
        const remcham = `${plugin.botAdmin || plugin.admin || plugin.group || plugin || noPrefix || hl || m.text.slice(0, 1) == hl || plugin.command}`;
        if (adminMode && !isOwner && !isROwner && m.isGroup && !isAdmin && remcham) return;
        ...
        if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
          mconn.conn.reply(m.chat, `No tienes más Diamantes, usa  *${usedPrefix}buyall* para comprar más`, m);
          continue; 
        }
        if (plugin.level > _user.level) {
          mconn.conn.reply(m.chat, `Necesitas el nivel *${plugin.level}* para usar este comando...`, m);
          continue; 
        }
        const extra = {
          match,
...
global.dfail = (type, m, conn) => {
  const msg = {
    ...
    unreg: '*¡Para Continuar con esta Función es necesario registrarse!*\n\n- /reg nombre.edad\n\n> *Uso Correcto:*: /reg RemCham.17',
    restrict: '*!Esta Función fué deshabilitado por mi Desarrollador*',
    }[type];
  const aa = {quoted: m, userJid: conn.user.jid};
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: packname, body: 'Hola ' + nombre, thumbnail: imagen1, sourceUrl: channel}}}}, aa);
