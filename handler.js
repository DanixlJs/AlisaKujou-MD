    ...
    for (const name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || '')[0])) {
        ...
        if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
          ...
          if (!['OWNER-unbanchat.js', 'INFO-contacto.js'].includes(name) && chat && chat?.isBanned && !isROwner) return; 
...
let about = (await this.fetchStatus(user).catch(console.error) || {}).status || '×'
        ...
        await m.conn.sendMessage(id, {text: `✧ ${user.split('@')[0]} seras expulsado del Grupo por protección.`, mentions: [user]}, {quoted: fkontak2});
                }
...
"thumbnail": apii.data, 
"title": `${action === 'add' ? '乂 BIENVENIDO 乂' : '乂 DESPEDIDA 乂'}`,
"body": global.wm,
"containsAutoReply": true,
"mediaType": 1, 
sourceUrl: global.link }}}, { quoted: fkontak })
apii.data = ''
}}}
    break;
    case 'promote':
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: global.wm, body: global.dev, thumbnail: global.img, sourceUrl: global.channel}}}}, aa);
